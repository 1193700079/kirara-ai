...
# from WhatsApp import Bot, ChatFullInfo, Update, User
# from WhatsApp.ext import Application, CommandHandler, ContextTypes, MessageHandler, filters
# from WhatsAppify_markdown import markdownify
...
class WhatsAppConfig(BaseModel):
    ...
    token: str = Field(description="WhatsApp 机器人的 Token，从 @BotFather 获取。")
    model_config = ConfigDict(extra="allow")

    def __repr__(self):
        return f"WhatsAppConfig(token={self.token})"


class WhatsAppAdapter(IMAdapter, UserProfileAdapter, EditStateAdapter, BotProfileAdapter):
    """
    WhatsApp Adapter，包含 WhatsApp Bot 的所有逻辑。
    """
    ...
    def __init__(self, config: WhatsAppConfig):
        ...
        self.logger = get_logger("WhatsApp-Adapter")
    ...
    async def convert_to_message(self, raw_message: Update) -> IMMessage:
        """
        将 WhatsApp 的 Update 对象转换为 Message 对象。
        :param raw_message: WhatsApp 的 Update 对象。
    ...
    async def query_user_profile(self, chat_sender: ChatSender) -> UserProfile:
        ...
        try:
            ...
            profile = UserProfile(  # type: ignore
                ...
                avatar_url=None,  # WhatsApp 需要额外处理获取头像
            )
